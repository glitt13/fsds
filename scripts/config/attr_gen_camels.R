...
main <- function(){
  ...
  path_config <- cmd_args[1] # path_config <- "~/git/formulation-selector/scripts/config/attr_gen_camels_config.yaml"
  raw_config <- yaml::read_yaml(path_config)
  dir_std_base <- raw_config$dir_std_base
  ...
  ds <- datasets
  path_metadata <- file.path(dir_metadata_out,glue::glue( "{dir_std_base}/{ds}/nldi_feat_{ds}_{ds_type}.csv}"))
